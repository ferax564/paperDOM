...
function endpointPosition(endpoint: Endpoint | undefined, elements: CanvasElement[]) {
  if (!endpoint) return { x: 0, y: 0 };
  if (endpoint.elementId) {
    const item = elements.find((e) => e.id === endpoint.elementId);
    if (item) {
      const { x, y, w, h } = item.frame;
      if (endpoint.anchor === "top") return { x: x + w / 2, y };
      if (endpoint.anchor === "right") return { x: x + w, y: y + h / 2 };
      if (endpoint.anchor === "bottom") return { x: x + w / 2, y: y + h };
      return { x, y: y + h / 2 };
    }
  }
...
function snapEndpoint(x: number, y: number, elements: CanvasElement[]): Endpoint {
  ...
  for (const item of elements) {
    ...
    const anchors: { anchor: Anchor; x: number; y: number }[] = [
      { anchor: "top", x: item.frame.x + item.frame.w / 2, y: item.frame.y },
      { anchor: "right", x: item.frame.x + item.frame.w, y: item.frame.y + item.frame.h / 2 },
      { anchor: "bottom", x: item.frame.x + item.frame.w / 2, y: item.frame.y + item.frame.h },
      { anchor: "left", x: item.frame.x, y: item.frame.y + item.frame.h / 2 },
    ];
...
export default function Home() {
  ...
  }, [guidePreferenceLoaded, smartGuidesEnabled]);
  ...
  const point = useCallback((clientX: number, clientY: number) => {
    ...
    return { x: Math.max(0, Math.min(PAGE_W, (clientX - rect.left) / scale)), y: Math.max(0, Math.min(PAGE_H, (clientY - rect.top) / scale)) };
  }, [scale]);
  ...
  const patchElement = useCallback((elementId: string, patch: Partial<CanvasElement>) => patchPage(pageId, (p) => ({ ...p, elements: p.elements.map((e) => e.id === elementId ? { ...e, ...patch } : e) })), [pageId, patchPage]);
  const patchFrame = useCallback((elementId: string, patch: Partial<Frame>) => patchPage(pageId, (p) => ({ ...p, elements: p.elements.map((e) => e.id === elementId ? { ...e, frame: { ...e.frame, ...patch } } : e) })), [pageId, patchPage]);
  ...
  const beginElementGesture = (event: React.PointerEvent, item: CanvasElement) => {
    ...
    gestureRef.current = { kind: "move", startX: event.clientX, startY: event.clientY, pageId: page.id, frames: Object.fromEntries(page.elements.filter((e) => ids.includes(e.id)).map((e) => [e.id, { ...e.frame }])) };
  };
  ...
  const addAt = useCallback((activeTool: Tool, x: number, y: number) => {
    if (activeTool === "text") {
      createTextBox(normalizeTextBoxFrame({ x, y }, { x, y }, PAGE_W, PAGE_H));
      return;
    }
  ...
  }, [createTextBox, page.id, patchPage]);
  ...
  useEffect(() => {
    const onMove = (event: PointerEvent) => {
      ...
      if (g.kind === "move") {
        ...
        setDoc((d) => ({ ...d, pages: d.pages.map((p) => p.id !== g.pageId ? p : ({ ...p, elements: p.elements.map((e) => {
          const f = g.frames[e.id]; return f ? { ...e, frame: { ...e.frame, x: f.x + dx, y: f.y + dy } } : e;
        }) })) }));
    ...
    const onUp = () => {
      ...
      if (g.kind === "marquee" && marquee) {
        const p = doc.pages.find((item) => item.id === g.pageId); if (p) setSelection(p.elements.filter((e) => !["connector", "line"].includes(e.type) && e.frame.x >= marquee.x && e.frame.y >= marquee.y && e.frame.x + e.frame.w <= marquee.x + marquee.w && e.frame.y + e.frame.h <= marquee.y + marquee.h).map((e) => e.id));
        setMarquee(null);
      }
      ...
      if (g.kind === "draw-text") {
        const frame = normalizeTextBoxFrame({ x: g.startX, y: g.startY }, { x: g.currentX, y: g.currentY }, PAGE_W, PAGE_H);
  ...
  }, [createTextBox, doc.pages, draftLine, marquee, patchPage, point, scale, smartGuidesEnabled]);
  ...
  const alignSelection = (mode: "left" | "center" | "top" | "middle") => {
    ...
    patchPage(page.id, (p) => ({ ...p, elements: p.elements.map((e) => !selection.includes(e.id) ? e : { ...e, frame: { ...e.frame, x: mode === "left" ? b.l : mode === "center" ? (b.l + b.r - e.frame.w) / 2 : e.frame.x, y: mode === "top" ? b.t : mode === "middle" ? (b.t + b.b - e.frame.h) / 2 : e.frame.y } }) }));
  };
  ...
  const deleteSelection = useCallback(() => {
    ...
    patchPage(pageId, (p) => ({
      ...p,
      elements: p.elements.filter((e) => !selection.includes(e.id) && !((e.from?.elementId && selection.includes(e.from.elementId)) || (e.to?.elementId && selection.includes(e.to.elementId)))),
    }));
    setSelection([]);
  ...
  const duplicateSelection = useCallback(() => {
    ...
    if (!source) return;
    const originals = source.elements.filter((e) => selection.includes(e.id));
    const map = new Map(originals.map((e) => [e.id, uid(e.type)]));
    const copies = originals.map((e) => ({
      ...e,
      id: map.get(e.id)!,
      name: `${e.name} copy`,
      frame: { ...e.frame, x: e.frame.x + 20, y: e.frame.y + 20 },
      from: e.from ? { ...e.from, elementId: e.from.elementId ? map.get(e.from.elementId) ?? e.from.elementId : undefined } : undefined,
      to: e.to ? { ...e.to, elementId: e.to.elementId ? map.get(e.to.elementId) ?? e.to.elementId : undefined } : undefined,
    }));
  ...
  const insertImageFile = useCallback((file?: File) => {
    ...
    reader.onload = () => {
      ...
      patchPage(page.id, (p) => ({ ...p, elements: [...p.elements, item] }));
      setSelection([id]);
      setTool("select");
    };
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Enter" && selectedOne && ["text", "shape", "ellipse"].includes(selectedOne.type)) { setEditingId(selectedOne.id); return; }
      ...
      if (["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(event.key) && selection.length) {
        ...
        patchPage(pageId, (p) => ({ ...p, elements: p.elements.map((e) => !selection.includes(e.id) ? e : { ...e, frame: { ...e.frame, x: e.frame.x + (event.key === "ArrowLeft" ? -delta : event.key === "ArrowRight" ? delta : 0), y: e.frame.y + (event.key === "ArrowUp" ? -delta : event.key === "ArrowDown" ? delta : 0) } }) }));
      }
    };
    ...
    <div className="editor-main">
      ...
      <section className={`workspace tool-${tool}`}>
        <div className="workspace-toolbar">
          ...
          <div className="canvas-status"><span>{page.name}</span><span>•</span><span>{PAGE_W} × {PAGE_H}</span></div>
        ...
        <div className="workspace-scroll" ref={workspaceScrollRef}><div className="workspace-stage" style={{ width: PAGE_W * scale, height: PAGE_H * scale }}><div ref={pageRef} className="page-canvas" style={{ width: PAGE_W, height: PAGE_H, background: page.background.color, transform: `scale(${scale})` }} onPointerDown={onCanvasDown}>
          <svg className="connector-layer" viewBox={`0 0 ${PAGE_W} ${PAGE_H}`} aria-hidden="true"><defs><marker id="arrowhead" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" fill="context-stroke" /></marker></defs>
      ...
      <aside className="inspector">
        ...
        {selectedOne ? <>
          ...
          {selectedOne.type === "image" && <section className="inspector-section"><div className="section-title">Image</div><label className="full-field"><span>Alt text</span><input value={selectedOne.content?.alt ?? ""} onChange={(e) => patchElement(selectedOne.id, { content: { ...selectedOne.content, alt: e.target.value } })} /></label><button className="replace-image-button" onClick={() => imageInputRef.current?.click()}><Upload size={14} /> Replace image</button></section>}
        ...
        <div className="inspector-footer"><div><span className="status-dot" /> Renderer healthy</div><span>{selection.length ? `${selection.length} selected` : "No selection"}</span></div>
      </aside>
    </div>
